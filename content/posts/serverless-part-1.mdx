...
Is short, a _serverless_ architecture or _serverless_ function can be use when one just wants to focus into the application and doesn't want to get into the complexities of setting and configuring a server.

In more layman terms, a _serverless_ architecture can be compared to **Service Apartments/Paying Guests** in real world. When people want place which is _safe_, _has all the basic necessities_, _has multiple accomodation options_, and _provides freedom to add some customisations_ they look for **Service Apartments/Paying Guests**. Similarly, in case of application development, if you don't want to deal with setting up servers, securing and configuring them, then serverless is the best option.

## AWS Serverless Application Model (AWS SAM)

There are a lot of ways you can create a serverless _Lambda_ function on **AWS**. A _Docker_ container with an <InlineCode>lambda_handler</InlineCode> pushed to AWS ECR can be used with Lambda.
...
1. Open a new _VS Code editor windown_ and based on your operating system use <InlineCode>cmd + shift + p</InlineCode> or <InlineCode>ctrl + shift + p</InlineCode> and search for **"AWS: Create Lambda SAM Application"** and select it
...
4. Name you application and save it inside a folder and export the folder to VS Code
...
Note: <InlineCode>application_name</InlineCode> referes to the name of the application you gave at the start.
...
This blog was more of a discovery and setup kind wherein we went through process of creating APIs using **<u>FastAPI</u>** and wrapping it into an adapter called **<u>Mangum</u>** and then deploying it as a Lambda using the _Cloudformation_ template.

In the next part we will create few tables inside **<u>DynamoDB</u>** and try to access it using the _Lambda_ function we created here
